            ...
            const numberDayStay =
              Math.abs(
                checkOutDay
                  .clone()
                  .endOf("days")
                  .diff(checkOutDay.clone().startOf("month"), "days")
              ) + 1; //cộng 1: tính cả ngày checkIn
            const waterPrice =
              ((roomData.waterPrice * roomData.person) / dayOfMon) *
              numberDayStay;
        ...
        if (orderData) {
          if (moment(orderData.expireTime).date() !== 6) {
            await orderModel.findOneAndUpdate(
              { _id: orderData._id },
              {
                expireTime: moment(orderData.expireTime)
                  .date(6)
                  .endOf("day")
                  .toDate(),
              }
      ...
      try {
        ...
        if (resData) {
          if (resData.isActived && !resData.isDeleted) {
            ...
            const dayOfMon = moment(checkOutDay).daysInMonth(); // số ngày của tháng
            const numberDayStay =
              Math.abs(endTime.clone().diff(startTime.clone(), "days")) + 1; //cộng 1: tính cả ngày checkIn
            const waterPrice =
              ((roomData.waterPrice * roomData.person) / dayOfMon) *
              numberDayStay;
            const servicePrice =
        ...
        if (orderData) {
          ...
          if (!orderData.isCompleted) {
            ...
            if (
              moment()
                .startOf("days")
                .diff(checkOutDayPlus3.clone().startOf("days")) <= 0
            ) {
              await global.agendaInstance.agenda.schedule(
                moment()
                  .add("1", "days")
              ...
              if (userData) {
                ...
                if (userData.email) {
                  ...
                  const mailOptions = {
                    ...
                    text: `Phòng ${resData.room.name} thuộc dãy ${
                      resData.motelRoom.name
                    } của quý khách sẽ hết hợp đồng vào ${checkOutDay
                      .clone()
                      .format("DD-MM-YYYY")}. Vui lòng truy cập trang web: ${
                      process.env.BASE_PATH_CLINET1
                    } thực hiện đăng nhập rồi vào đường dẫn ${
                      process.env.BASE_PATH_CLINET3
                    }job-detail/${resData._id}/${
                      resData.room._id
                  ...
                  console.log(
                    `User id: ${userData.user} không được tìm thấy hoặc chưa cập nhật email`
                  );
